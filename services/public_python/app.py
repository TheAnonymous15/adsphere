...
from python_system.models import Base, Company, Ad, Category, AdView, Interaction, Favorite, get_current_timestamp
from python_system.database import engine, SessionLocal, get_db

# ============================================================================
# PATHS CONFIGURATION
# ============================================================================
BASE_PATH = Path(__file__).parent.parent.parent  # /adsphere
SERVICE_PATH = Path(__file__).parent  # /adsphere/services/public_python
ASSETS_PATH = BASE_PATH / "services" / "assets"
TEMPLATES_PATH = SERVICE_PATH / "templates"
INCLUDES_PATH = BASE_PATH / "services" / "public" / "includes"
COMPANY_DATA_PATH = BASE_PATH / "services" / "company" / "data"  # Ad media storage
APP_COMPANY_DATA_PATH = BASE_PATH / "app" / "companies" / "data"  # Alternative media storage
...
# Mount static files
app.mount("/services/assets", StaticFiles(directory=str(ASSETS_PATH)), name="assets")

# Mount company data for ad media
if COMPANY_DATA_PATH.exists():
    app.mount("/services/company/data", StaticFiles(directory=str(COMPANY_DATA_PATH)), name="company_data")
if APP_COMPANY_DATA_PATH.exists():
    app.mount("/app/companies/data", StaticFiles(directory=str(APP_COMPANY_DATA_PATH)), name="app_company_data")
...
def get_ads_from_db(db: Session, category: str = None, search: str = None, limit: int = 20, offset: int = 0):
    ...
    for ad in ads:
        ...
        if ad.media_path:
            ...
            # URL should be: /services/company/data/category/company/ad_id/filename
            media_url = f"/services/company/data/{ad.media_path}"
...
async def api_get_ad(ad_id: str, db: Session = Depends(get_db)):
    ...
    # Build proper media URL
    media_url = f"/services/company/data/{ad.media_path}" if ad.media_path else ""
