...
// Only configure session if not already started (handled by index.php)
...
function checkLoginRateLimit($db, $username, $ip, $config) {
    $now = time();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!checkIPWhitelist($securityConfig['ip_whitelist'])) {
        ...
        $rateCheck = checkLoginRateLimit($db, $username, $ip, $securityConfig);
