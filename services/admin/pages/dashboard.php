...
if (!$isSuperAdmin) {
    // Redirect non-super admins to company portal
    header("Location: http://localhost:8003/dashboard");
    exit();
}
?>
        ...
        <div class="glass-card rounded-2xl p-6 mb-12">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="/moderation" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg transition text-sm">
...
async function loadLiveStats() {
    ...
    try {
        ...
        const [adsRes, analyticsRes] = await Promise.all([
            fetch('/services/api/get_ads.php'),
            fetch('/services/api/get_analytics.php')
        ]);
...
async function loadActivityFeed() {
    try {
        const response = await fetch('/services/api/get_ads.php');
...
async function loadUsersData() {
    ...
    try {
        ...
        // TODO: Create /services/api/admin/get_users.php endpoint
        const response = await fetch('/services/api/admin/get_users.php');
        const data = await response.json();
...
async function loadCompaniesData() {
    ...
    try {
        ...
        const response = await fetch('/services/api/get_companies.php');
        const data = await response.json();
...
async function suspendCompany(companySlug) {
    ...
    try {
        ...
        const response = await fetch('/services/api/update_company_status.php', {
            method: 'POST',
            body: formData
        });
...
async function loadDevicesData() {
    ...
    try {
        ...
        const response = await fetch('/services/api/user_profiling.php');
        const data = await response.json();
    ...
    if (!menu) {
        ...
        menu.innerHTML = `
            ...
            <div class="p-2">
                ...
                <a href="/logout" class="flex items-center gap-3 px-4 py-3 hover:bg-red-600/20 rounded-lg transition text-red-400">
...
async function loadAdStatusStats() {
    try {
        const res = await fetch('/services/api/ad_status_stats.php');
...
function filterAdsByStatus(status) {
    ...
    // Example: window.location.href = `/admin/ads.php?status=${status}`;
}
...
async function loadViolations() {
    try {
        const [violationsRes, statsRes] = await Promise.all([
            fetch('/services/api/moderation_violations.php?action=list&status=pending'),
            fetch('/services/api/moderation_violations.php?action=stats')
        ]);
...
async function takeViolationAction(violationId, actionType, adId) {
    ...
    try {
        ...
        const res = await fetch('/services/api/moderation_violations.php', {
            method: 'POST',
            body: formData
        });
...
async function runNewScan() {
    ...
    try {
        const res = await fetch('/services/api/scanner.php?action=scan');
        const data = await res.json();
