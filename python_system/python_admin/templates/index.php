 ...
 *
 * Start: php -S localhost:8002 -t python_admin
 * ============================================================================
 */
...
define('BASE_PATH', dirname(__DIR__));  // python_system directory

// Include shared utilities
require_once BASE_PATH . '/python_shared/bootstrap.php';
...
switch (true) {
    ...
    case strpos($requestUri, '/static/') === 0:
    case strpos($requestUri, '/assets/') === 0:
        $assetPath = $requestUri;
        // Normalize path - both /static/ and /assets/ point to same location
        if (strpos($assetPath, '/static/') === 0) {
            $assetPath = str_replace('/static/', '/assets/', $assetPath);
        }
