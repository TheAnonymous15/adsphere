...
<nav class="bg-slate-900/80 backdrop-blur-lg border-b border-white/10 sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            ...
            <div class="hidden md:flex items-center gap-6">
                ...
                <a href="/upload" class="text-gray-300 hover:text-white transition">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="relative">
                    ...
                    <div id="userMenuDropdown" class="hidden absolute right-0 mt-2 w-48 bg-slate-800 rounded-lg shadow-xl border border-white/10 py-2">
                        <a href="/profile" class="block px-4 py-2 hover:bg-white/10 transition">
                            <i class="fas fa-user mr-2"></i>Profile
                        </a>
                        ...
                        <a href="/logout" class="block px-4 py-2 hover:bg-red-500/20 text-red-400 transition">
    ...
    <div id="mobileMenu" class="hidden md:hidden bg-slate-800 border-t border-white/10">
        ...
        <a href="/upload" class="block px-4 py-3 hover:bg-white/10 transition">
            <i class="fas fa-plus-circle mr-2"></i>New Ad
        </a>
        <a href="/profile" class="block px-4 py-3 hover:bg-white/10 transition">
            <i class="fas fa-user mr-2"></i>Profile
        </a>
    </div>
</nav>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div id="overview" class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 sm:p-8 mb-8 shadow-xl">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <a href="/upload" class="bg-white text-indigo-600 px-6 py-3 rounded-lg font-semibold hover:shadow-lg transition flex items-center gap-2">
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            <a href="/upload" class="bg-blue-600 hover:bg-blue-700 p-4 rounded-xl text-center transition group">
                <i class="fas fa-plus-circle text-3xl mb-2 group-hover:scale-110 transition"></i>
                <p class="font-semibold">Post Ad</p>
            </a>
            <a href="/ads" class="bg-green-600 hover:bg-green-700 p-4 rounded-xl text-center transition group">
                <i class="fas fa-list text-3xl mb-2 group-hover:scale-110 transition"></i>
                <p class="font-semibold">My Ads</p>
            </a>
            <a href="/profile" class="bg-purple-600 hover:bg-purple-700 p-4 rounded-xl text-center transition group">
    ...
    <section id="my-ads" class="mb-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="/ads" class="text-indigo-400 hover:text-indigo-300 text-sm">
        ...
        <!-- Ads Container -->
        <div id="myAdsContainer" class="hidden">
            <!-- JS will populate as grid -->
        </div>
        ...
        <div id="emptyState" class="hidden bg-slate-800/30 rounded-xl p-12 text-center">
            ...
            <a href="/upload" class="inline-block bg-indigo-600 hover:bg-indigo-700 px-6 py-3 rounded-lg font-semibold transition">
...
async function loadDashboardData() {
    try {
        ...
        const [adsRes, statsRes] = await Promise.all([
            fetch(`/api/get_ads?company=${encodeURIComponent(companySlug)}`),
...
async function updateLiveActivity() {
    try {
        const response = await fetch('/api/live_activity');
...
async function loadContactAnalytics() {
    ...
    try {
        const response = await fetch(`/api/contact_analytics?days=${dateRange}`);
        const data = await response.json();
...
function renderAds(ads) {
    ...
    container.innerHTML = latestAds.map(ad => {
        ...
        const views = ad.views || ad.analytics?.views || 0;
            ...
            <div class="ad-card bg-slate-800/50 backdrop-blur rounded-xl overflow-hidden border border-white/10 hover:border-indigo-500/50 transition">
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center justify-between text-xs text-gray-500 mb-3">
                        ...
                        <span><i class="fas fa-eye mr-1"></i>${views} views</span>
                    </div>
...
function updateAnalytics(ads) {
    ...
    const sortedAds = [...ads].sort((a, b) => (b.views || 0) - (a.views || 0)).slice(0, 3);
    ...
    if (sortedAds.length === 0) {
        ...
        topAds.innerHTML = sortedAds.map((ad, index) => `
            <div class="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-700/50 transition">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium truncate">${escapeHtml(ad.title)}</p>
                    <p class="text-xs text-gray-400">${ad.views || 0} views</p>
                </div>
            </div>
        `).join('');
    }
...
async function deleteAd(adId, title) {
    ...
    try {
        ...
        if (data.success) {
            ...
            showNotification(data.error || 'Failed to delete ad', 'error');
        }
    } catch (error) {
...
function showNotification(message, type = 'info') {
    ...
    notification.className = `fixed top-4 right-4 ${bgColor} text-white px-6 py-3 rounded-lg shadow-xl z-50 flex items-center gap-2 slide-in`;
